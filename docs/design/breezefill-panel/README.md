# Handoff: BreezeFill — Chrome side-panel redesign

## Overview

BreezeFill is a Chrome MV3 **side panel** used by Singapore GPs while an insurer's
online claim form is open in the tab beside it. The doctor types the patient's
name, pastes their consultation note, BreezeFill extracts the patient's
...
inferred, and BreezeFill writes the accepted values into the insurer's own page.
**It never submits.**
...
- Title `BreezeFill` 15px/500/−0.015em; tagline **"Fills the form in place. Never
...
- Subtitle **"Type the patient's name yourself. BreezeFill never guesses this
  one."** 12.5px `#86868B`.
...
- Title; subtitle **"BreezeFill has no access to any page until you grant it here,
...
- `logo-mark.svg` — the BreezeFill mark, described above. Needs 16/32/48/128 PNG
...
| File | What it is |
|---|---|
| `BreezeFill Panel v2.dc.html` | **The implementation target.** The full progressive-disclosure panel with real interaction: name → note → find details → verify gate → map → review → fill, plus the typeface switcher (a design tool, not a product feature). |
| `BreezeFill Panel.dc.html` | The earlier single-scroll version, kept for reference on the review list and the fill report. |
| `BreezeFill Logo.dc.html` | Logo construction, size ladder, tile treatments, lockups, and the mark shown in the panel header. |
| `BreezeFill Theme Directions.dc.html` | How the palette and typeface were chosen (turns 1–5). Turn 5, option 5b is the direction that shipped. |
